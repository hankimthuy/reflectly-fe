...
- **Dải "đang đọc tâm trạng" (mood-read ribbon):** hiển thị phía trên khung chat bằng 1 thanh trượt nhẹ→nặng (theo gradient tâm trạng chung của app) và nhãn cảm xúc lúc mở đầu phiên. Nay dựa trên `moodEmotion` **do backend tự chấm cho từng tin nhắn** — giá trị này được lưu lại (không còn mất đi khi rời trang: xem `initialMoodEmotion`/`finalMoodEmotion` trên Conversation, mục 6). Chỉ ngay sau khi vừa gửi một tin nhắn, trong lúc chờ backend phản hồi, dải tạm hiển thị 1 ước lượng quét từ khoá tính phía frontend rồi tự thay bằng giá trị thật ngay khi có phản hồi.
- Từ Aura Soft v2, dải này gộp lại còn **đúng một hàng** (nhãn "Đang đọc" — thanh trượt — tên cảm xúc). Hàng thang đo bên dưới **bị bỏ**, kéo theo 2 thông tin không còn hiển thị ở màn Trò chuyện: **cảm xúc lúc mở đầu phiên** và **số phút đã trôi qua**. Cảm xúc đầu/cuối phiên vẫn được lưu và vẫn xem được ở màn Phiên đã lưu (`initialMoodEmotion`/`finalMoodEmotion`, mục 6).
...
- **Dải "đang đọc tâm trạng" (mood-read ribbon) ở Trò chuyện**: từ khi có `moodEmotion`/`moodScore` do backend chấm theo tin nhắn (mục 4.4, mục 6), dải này không còn là suy luận thuần phía frontend/không lưu lại như trước — chỉ còn khoảng hở rất ngắn ngay sau khi gửi một tin nhắn (trước khi có phản hồi) là còn dùng tạm ước lượng quét từ khoá phía frontend.

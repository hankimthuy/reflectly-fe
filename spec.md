...
- Backend: `hankimthuy/reflectly-be` — Spring Boot API + AI Coach engine (gọi model qua OpenRouter).
...
- **Giao diện:** một hệ thống thiết kế Tailwind CSS duy nhất áp dụng thống nhất cho toàn bộ trang — token màu/typography/bo góc định nghĩa tập trung ở `src/styles/tailwind.css`, các trang dùng chung tên biến/class (ví dụ `--color-accent`, `--color-pine`, `.btn`, `.tag`), nên đổi cả bộ nhận diện (màu, font, hình khối) chỉ cần sửa file token này. Hệ token `coach-*` cũ đã được gỡ bỏ hoàn toàn ở Giai đoạn 4 (xem mục 1.1). Riêng lớp SCSS cũ (`src/styles/_variables.scss` + `src/index.scss`) **vẫn còn tồn tại** và vẫn tạo style mặc định cho các thẻ trần (`button`, `a`, `h1`–`h3`, `.container`, `.main-content`) thông qua bộ biến `--c-*` — nên bộ `--c-*` này được trỏ về đúng bảng màu/font hiện hành của hệ thống thiết kế để không có màu cũ lọt ra ở những phần tử không đi qua `.btn`/`.tag`/class Tailwind. Các token `--garden-*` là tư liệu chết, không còn được tham chiếu.
- **Đa ngôn ngữ:** i18next, 2 ngôn ngữ VI (mặc định)/EN, một số namespace tiếng Anh chưa dịch đầy đủ.
- **Mobile:** breakpoint dưới 500px được coi là điện thoại (`_responsive.scss`'s `mobile-only`/499px cũng như các `@media (max-width: 499px)` rải rác trong `tailwind.css`/component scss). `touch-action: manipulation` áp cho `body` và bỏ viền chạm xám mặc định của iOS Safari cho mọi `button`/`a`/`[role="button"]` (2026-09-17) để loại độ trễ chạm ~300ms trên di động. `viewport-fit=cover` bật ở `index.html` để `env(safe-area-inset-bottom)` hoạt động — dùng ở thanh tab dưới cùng và khung soạn tin/panel Catch dạng sheet để không bị vùng cử chỉ hệ điều hành che mất.
...
|---|---|---|---|
| `/` | Công khai | Trang chủ marketing | Hero giới thiệu sản phẩm (kèm 1 đoạn hội thoại mẫu minh hoạ) + khối **4 điều Aura làm** (không phải dữ liệu thật, chỉ minh hoạ): Trò chuyện, Giữ lại (Catch), Tấm gương của bạn (Mirror), Những người trong đó (People). Dành cho khách chưa đăng nhập. |
...
- Nội dung: lời chào theo khung giờ (sáng/chiều/tối) + chuỗi ngày liên tục ghi nhật ký; 1 câu gợi mở trò chuyện xoay vòng mỗi ngày (ổn định trong ngày, không lặp lại y hệt hôm sau) với 2 lối vào Trò chuyện ("Bắt đầu tại đây" mang theo câu gợi mở làm tin nhắn đầu, hoặc "Có chuyện khác đang nghĩ tới" vào chat trống); dải cột thể hiện cảm xúc mỗi ngày trong 7 ngày qua — nay lấy từ endpoint tổng hợp tâm trạng phía backend `GET /users/mood-summary` (mục 7) thay vì tự tính từ nhãn cảm xúc trên entry như trước, kèm 1 dòng nhận xét xu hướng ngắn tính phía frontend (nhẹ hơn / gần như không đổi / nặng hơn so với đầu giai đoạn 7 ngày); danh sách "Tiếp tục từ đây" — tối đa 3 mục gần nhất (phiên chat đã kết thúc hoặc nhật ký), mới nhất trước; xem trước Tấm gương (số dòng đã "giữ lại" ở mỗi ô Cởi mở/Điểm mù/Giữ kín/Chưa rõ — xem mục 4.7); danh sách rút gọn tối đa 2 người "cần bạn để ý" từ Bản đồ Mối quan hệ (mục 4.6, kèm gợi ý nhắc nhở nếu có); lối tắt viết nhật ký một mình (không qua chat).
- Số liệu "Tuần này" (số phiên chat + số nhật ký trong 7 ngày) hiển thị cố định ở khung điều hướng bên trái (desktop).
- Từ Aura Soft v2, màn này theo nguyên tắc "chỉ nhãn, bỏ câu diễn giải": (1) **dòng chuỗi ngày liên tục dưới lời chào bị bỏ** — số liệu chuỗi ngày vẫn còn, nhưng chỉ ở "Tôi" (mục 4.12); (2) mỗi ô Tấm gương chỉ còn nhãn + số, bỏ dòng mô tả nhỏ ("cả hai đều biết", "người khác thấy"...) — vẫn còn ở tab Tấm gương (mục 4.7); (3) danh sách "cần bạn để ý" bỏ câu gợi ý nhắc nhở (`nudgeText`) — câu này vẫn hiển thị ở thẻ chi tiết từng người trong tab Mọi người (mục 4.6); (4) mỗi dòng "Tiếp tục từ đây" bỏ nhãn phân loại phiên chat/nhật ký, chỉ còn ngày + tiêu đề.
...
- **Nút quay lại (chỉ trên di động, thêm 2026-09-17):** mũi tên `<` ở góc trên bên trái, đưa về "Hôm nay" nhưng **không** kết thúc phiên — phiên vẫn ACTIVE và mở lại được. Chỉ hiện trên di động vì trên desktop sidebar luôn có sẵn; trên di động, thanh tab dưới cùng bị ẩn ở màn này để nhường chỗ cho khung soạn tin (xem mục 5 — "Nút Chat nổi toàn cục"/full-bleed layout), nên trước khi có nút này, cách duy nhất rời màn là bấm "Kết thúc phiên" hoặc nút back của trình duyệt.
- Gửi tin nhắn (`POST /conversations/:id/messages`) → nhận về **cả 2** tin nhắn cùng lúc: tin nhắn của người dùng vừa gửi và phản hồi của AI (`{ userMessage, assistantMessage }`) — mỗi tin nhắn kèm theo cảm xúc do backend tự chấm (`moodEmotion`, `moodScore`, xem mục 6). Trước đây endpoint này chỉ trả về tin nhắn AI.
- **Dải "đang đọc tâm trạng" (mood-read ribbon) — đã gỡ (2026-09-17):** từng hiển thị phía trên khung chat bằng 1 thanh trượt nhẹ→nặng, nhưng không phải một điều khiển thật (không bấm/kéo được) — chỉ là một chỉ báo trực quan. Người dùng phản ánh rằng nó trông giống một thanh trượt có thể thao tác nhưng lại không làm gì, nên đã bỏ khỏi màn Trò chuyện. Cảm xúc đầu/cuối phiên vẫn được backend chấm và lưu, vẫn xem được ở màn Phiên đã lưu (`initialMoodEmotion`/`finalMoodEmotion`, mục 6) — chỉ riêng khung chat trực tiếp không còn hiển thị chỉ báo này nữa.
- Có thể yêu cầu tóm tắt phiên (dạng văn bản markdown, hiển thị bằng renderer markdown).
- **Thời gian chờ:** 2 lời gọi thực sự chạy model ở backend — gửi tin nhắn và yêu cầu tóm tắt — được cho phép chờ tới **60 giây** thay vì mốc 10 giây mặc định của toàn bộ API. Trước đây 2 lời gọi này bị huỷ phía trình duyệt khi model trả lời chậm, và người dùng chỉ thấy "Aura không trả lời".
...
- **Chữ + icon cho 3 nút thao tác dưới khung soạn tin (sửa 2026-09-17):** "Catch"/"Summarize"/"End session", mỗi nút kèm icon riêng (bookmark/sparkles/log-out). Trước đó nút Catch ghi "⌘K to catch a line" — nhắc tới một phím tắt **chưa bao giờ thực sự được lập trình** (không có trình lắng nghe bàn phím nào cho ⌘K), gây hiểu lầm là nút hỏng, đặc biệt vô nghĩa trên di động (không có bàn phím vật lý); nút kết thúc phiên cũ ghi "End & keep the shift" — người dùng phản ánh khó hiểu, đổi thành "End session"/"Kết thúc phiên".
...
- Mỗi người có: tên, loại quan hệ (Gia đình/Bạn bè/Người yêu/Đồng nghiệp/Quản lý/Khác), ghi chú, thời điểm được nhắc tới gần nhất, và một **chỉ số sức khoẻ mối quan hệ (healthSignal, thang 0.0–1.0)**, kèm gợi ý nhắc nhở (nudge text) nếu có.
- Nút của mỗi người được tô theo **4 mức**, có chú giải (legend) ngay dưới sơ đồ: **cần để ý** và **đang xa dần** (2 mức thấp của healthSignal) dùng màu cảnh báo riêng — không dùng chung màu với nút hành động chính; phần còn lại tách đôi theo số ngày kể từ lần được nhắc tới gần nhất (`daysSinceLastMention`): **được nhắc tới nhiều nhất** (vừa được nhắc trong vài ngày gần đây) và **ổn định** (mặc định). Mức thứ 4 này là thông tin mới so với trước đây — nó cho biết ai đang chiếm nhiều tâm trí người dùng nhất, chứ không phải ai đang có vấn đề.
...
- Thống kê cá nhân nay lấy từ endpoint tổng hợp phía backend `GET /users/stats` (mục 7): số ngày liên tục ghi nhật ký (streak), cảm xúc xuất hiện nhiều nhất, số phiên chat + số nhật ký, biểu đồ phân bố cảm xúc trong 90 ngày gần nhất — tính toán phía frontend từ dữ liệu entries (`statsUtil.ts`) chỉ còn dùng làm giá trị tạm trong lúc chờ endpoint trả về, không còn là nguồn số liệu chính.
...
- Dành cho khách chưa đăng nhập: hero + khối **4 điều Aura làm** minh hoạ tính năng — **Trò chuyện**, **Giữ lại** (Catch), **Tấm gương của bạn** (Mirror), **Những người trong đó** (People/PRM). Đây là **nội dung tĩnh/minh hoạ**, không phải dữ liệu thật của người dùng.
- Từ Aura Soft v2, trang này được cắt gọn: (1) còn 4 khối thay vì 6 — **Dòng thời gian** được gộp vào câu chuyện của Catch ("giữ lại một câu → thành một mẫu hình"), còn **Giá trị cốt lõi** chuyển hẳn về Onboarding (mục 4.2) thay vì xuất hiện trong lời chào hàng; (2) khối "Leading Self" 4 lớp (Nền tảng / Innerverse / ... ) **bị bỏ khỏi trang** — đây là mô hình nội bộ, không phải điều khách truy cập cần biết; (3) ảnh minh hoạ trong hero được thay bằng **xem trước một đoạn trò chuyện** (1 câu của Aura, 1 câu của người dùng) dựng đúng hình dạng màn Trò chuyện thật — bản xem trước đã bỏ dòng "đang đọc tâm trạng" cùng đợt gỡ dải này khỏi màn Trò chuyện thật (2026-09-17).
- Lưu ý: bỏ khối "Leading Self" chỉ là **thay đổi nội dung marketing**, không gỡ tính năng nào — Dòng thời gian (mục 4.8) và Giá trị cốt lõi (mục 4.11) vẫn hoạt động đầy đủ trong app.
...
| **Conversation** | Một phiên chat với Aura | `id, status (ACTIVE/ENDED/EXTRACTING/EXTRACTED/EXTRACTION_FAILED), startedAt, endedAt?, messages[], summary?, initialMoodEmotion?, initialMoodScore?, finalMoodEmotion?, finalMoodScore?` — 4 field tâm trạng cuối chỉ có giá trị sau khi phiên đã kết thúc và có ít nhất 1 tin nhắn được chấm điểm |
| **ConversationMessage** | Một tin nhắn trong phiên chat | `id, role (USER/ASSISTANT), content, createdAt, moodEmotion?, moodScore?` — cảm xúc do backend tự chấm cho tin nhắn đó (nullable) |
| **MoodSummaryDay / UserStats** | Không phải entity lưu trữ — là 2 kiểu dữ liệu trả về từ 2 endpoint tổng hợp phía backend (mục 7), dùng để vẽ dải 7 ngày ở "Hôm nay" và 3 ô thống kê + biểu đồ cảm xúc ở "Tôi" | `MoodSummaryDay: date, score?, emotion?, hasData` · `UserStats: currentStreakDays, mostFrequentEmotion?, mostFrequentEmotionCount?, talksCount, entriesCount, emotionsWindowDays, emotionDistribution[]` |
...
| Người dùng | `GET/PUT /users/profile`, `PUT /users/password`, `POST /users/avatar`, `PUT /users/onboarding`, `GET /users/mood-summary?days=&tz=`, `GET /users/stats?days=&tz=` | Xem/sửa hồ sơ, đổi mật khẩu, tải avatar, hoàn tất onboarding; **mới** — tâm trạng theo ngày (mặc định 7 ngày, dùng cho "Hôm nay") và thống kê tổng hợp (mặc định 90 ngày, dùng cho "Tôi") theo múi giờ trình duyệt |
| Nhật ký | `GET/POST /entries`, `GET/PUT/DELETE /entries/:id` | CRUD nhật ký |
| Trò chuyện (Coach) | `POST /conversations`, `GET /conversations`, `GET /conversations/:id`, `POST /conversations/:id/messages`, `POST /conversations/:id/end`, `POST /conversations/:id/summarize` | Toàn bộ vòng đời một phiên chat Aura — **thay đổi breaking:** `POST .../messages` nay trả về `{ userMessage, assistantMessage }` thay vì chỉ tin nhắn AI như trước; `GET /conversations`, `GET /conversations/:id` và `POST /conversations/:id/end` nay có thêm 4 field tâm trạng đầu/cuối phiên trên response (xem mục 6) |
...
- **Logo/wordmark chính thức của thương hiệu "Aura Self AI"**: đã có — `public/logo-mark.png` (từ bộ thiết kế Aura Soft v2) nay là logo chính thức, dùng ở favicon, wordmark (khung điều hướng, header trang công khai, đăng nhập/đăng ký, 404) và làm **gương mặt của Aura** cạnh mỗi câu Aura nói (bong bóng chat, thẻ gợi mở ở "Hôm nay", tóm tắt phiên). Còn lại: ảnh GIF linh vật cũ (`src/assets/aura/aura-idle.gif` + component `MimoCharacter`) nay **không còn được dùng ở đâu** — nên xoá ở lần dọn dẹp tiếp theo.
...
| 2026-09-11 | Claude (agent) | Nền tảng bộ nhận diện trực quan mới **"Aura Soft"** (thay "Hearth"), giai đoạn 1/2 — chỉ token + component dùng chung, chưa restyle từng trang (nhánh `claude/epic-fermat-78xg34`). Thay đổi **nghiệp vụ** duy nhất: Bản đồ Mối quan hệ (mục 4.6) chuyển từ **3 mức màu** sang **4 mức**, tách mức "ổn định" cũ thành "được nhắc tới nhiều nhất" (theo `daysSinceLastMention`) và "ổn định", kèm chú giải 4 mục — thêm khoá i18n `reflect.people.talkedAboutMost` ở cả `en.json`/`vi.json`. Ngoài ra tách bạch vai trò màu: màu nhấn (indigo) nay **chỉ** dành cho hành động chính, còn tín hiệu "cần để ý" (nút người cần chú ý ở Today, biến thể nút `danger`, thông báo lỗi) dùng màu cảnh báo riêng (clay). Không đổi route, model dữ liệu hay endpoint backend. Sửa lại mục 2: nhận định cũ rằng hệ SCSS/`--c-*` "đã được thay thế hoàn toàn" là **sai** — lớp này vẫn còn và vẫn style các thẻ trần, nay đã được trỏ về bảng màu/font hiện hành. |
| 2026-09-11 | Claude (agent) | Hoàn thiện "Aura Soft" giai đoạn 2/2 — restyle toàn bộ các trang còn lại theo token đã có, cùng nhánh `claude/epic-fermat-78xg34`. Thay đổi **nghiệp vụ**: (1) **breaking** — `POST /conversations/:id/messages` nay trả về `{ userMessage, assistantMessage }` thay vì chỉ tin nhắn AI; mỗi tin nhắn và mỗi phiên chat (`GET /conversations`, `GET /conversations/:id`, `POST /conversations/:id/end`) nay kèm cảm xúc do backend tự chấm (`moodEmotion`/`moodScore`, `initialMoodEmotion`/`finalMoodEmotion`...) — xem mục 4.4, mục 6; (2) tích hợp 2 endpoint mới `GET /users/mood-summary` (dải 7-ngày ở "Hôm nay", mục 4.3) và `GET /users/stats` (3 ô thống kê + biểu đồ cảm xúc ở "Tôi", mục 4.12) — thay thế vai trò nguồn-sự-thật của tính toán phía frontend cũ (`statsUtil.ts` chỉ còn là fallback tạm khi đang tải); (3) dải "đang đọc tâm trạng" ở Trò chuyện không còn chỉ là suy luận tạm thời/không lưu — nay dùng giá trị backend đã chấm và được lưu lại (mục 8). Kèm 1 đợt dọn copy nhỏ: vài chỗ còn sót chữ "Coach" trong nội dung hiển thị (rỗng bản đồ Mọi người, rỗng Dòng thời gian, thông báo khi bỏ trống bước thêm người ở Onboarding) đổi thành "Aura" — không đổi nghiệp vụ, chỉ đổi câu chữ. Không đổi route hay model dữ liệu nào khác ngoài các field tâm trạng nêu trên. **Cần cập nhật chéo:** `reflectly-fe`/`reflectly-be` đã đồng bộ theo hợp đồng API mới này; không cần sửa gì thêm ở `reflectly-be/spec.md` (repo đó tự quản lý tài liệu của nó). |
| 2026-09-11 | Claude (agent) | Dựng bộ thiết kế **Aura Soft v2** (`AuraSelfAI v2.dc.html`, bản giao từ Claude Design; nhánh `claude/auraselai-v2-design-0bedba`). Token màu/font/bo góc của v2 **đã trùng khớp sẵn** với hệ thống có từ giai đoạn 2/2, nên lần này chỉ là thay đổi **bố cục và nội dung theo từng màn**. Thay đổi nghiệp vụ/nội dung: (1) **Trang chủ công khai** (mục 3, 4.14) cắt từ 6 khối tính năng xuống **4** — Dòng thời gian gộp vào Catch, Giá trị cốt lõi chuyển về Onboarding — và **bỏ hẳn khối "Leading Self" 4 lớp**; ảnh minh hoạ trong hero thay bằng xem trước một đoạn trò chuyện. Cả 2 tính năng bị rút khỏi trang chào hàng **vẫn hoạt động đầy đủ trong app**. (2) **"Hôm nay"** (mục 4.3) bỏ 4 chỗ chữ diễn giải: dòng chuỗi ngày dưới lời chào, mô tả nhỏ trong 4 ô Tấm gương, câu gợi ý nhắc nhở ở danh sách "cần để ý", và nhãn phân loại phiên chat/nhật ký ở "Tiếp tục từ đây" — mọi thông tin này vẫn còn ở màn chuyên trách tương ứng. (3) **Trò chuyện** (mục 4.4) gộp dải "đang đọc tâm trạng" còn 1 hàng, bỏ hàng thang đo — kéo theo cảm xúc lúc mở phiên và số phút đã trôi qua không còn hiển thị ở màn này (vẫn lưu, vẫn xem được ở Phiên đã lưu). (4) **Logo chính thức đã có** (mục 8): `public/logo-mark.png` dùng làm favicon, wordmark và gương mặt của Aura cạnh mỗi câu Aura nói — khép lại khoảng trống "thiếu logo/wordmark"; ảnh GIF linh vật cũ nay thành code chết. Không đổi route, model dữ liệu hay endpoint backend nào. Dọn i18n: bỏ **42 khoá** đã mồ côi và thêm 3 khoá mới cho đoạn hội thoại mẫu ở hero, đồng bộ ở cả `en.json`/`vi.json` (374 → 335 khoá lá, 2 file khớp tuyệt đối). Sửa mục 8: ghi chú cũ về khoá mồ côi `brand.contactLink` đã lỗi thời — khoá này không còn trong `vi.json`. |
| 2026-09-17 | Claude (agent) | Sửa lỗi khiến **toàn bộ màn "Hôm nay" và "Trò chuyện" không bấm được sau khi đăng nhập**: hook `useSidebarFooter` nhận thẳng JSX nên effect chạy lại ở mọi lần render, cộng với giá trị context của `AppShell` được tạo mới mỗi lần render, tạo vòng lặp cập nhật vô hạn ("Maximum update depth exceeded") — React dừng render giữa chừng, màn Trò chuyện không dựng được ô nhập/nút nào. Hook nay nhận một hàm dựng + danh sách phụ thuộc, và context được ghi nhớ (memo). Ngoài ra nới thời gian chờ lên 60 giây cho 2 lời gọi có chạy model (`POST /conversations/:id/messages`, `POST /conversations/:id/summarize`) vì mốc 10 giây mặc định huỷ chúng giữa chừng — xem mục 4.4. Không đổi route, model dữ liệu hay endpoint. **Ghi chú chéo:** `reflectly-be` cùng đợt này đã chuyển engine AI từ Gemini sang OpenRouter, không đổi hợp đồng API nào. |
| 2026-09-17 | Claude (agent) | Đợt 2 cùng ngày — người dùng báo bấm không được gì trên di động thật (bản chưa deploy fix trước đó lên `main`). Gỡ hẳn dải "đang đọc tâm trạng" (mood-read ribbon) ở Trò chuyện theo yêu cầu — chỉ là chỉ báo trực quan, không bấm/kéo được, gây hiểu lầm là một điều khiển hỏng; xoá luôn `useLiveMoodRead`, `readMoodFromText`/`MOOD_KEYWORDS`/`moodBucket` (chết theo), và dòng xem trước tương ứng ở trang chủ công khai — xem mục 3, 4.4, 4.14, 5. Cảm xúc đầu/cuối phiên do backend chấm (`initialMoodEmotion`/`finalMoodEmotion`) không đổi, chỉ riêng khung chat trực tiếp không hiển thị nữa. Kèm một đợt sửa mobile cụ thể (mục 2): `.input`/ô soạn tin bump lên 16px (dưới mức đó iOS Safari tự zoom khi focus, gây lệch mọi cú chạm sau đó), `touch-action: manipulation` + bỏ viền chạm xám mặc định (giảm ~300ms độ trễ chạm), vùng chạm lớn hơn cho các nút dạng `.tag`/nút "Catch this"/nút Lưu-Huỷ ở form Người trên di động, và `env(safe-area-inset-bottom)` cho thanh tab dưới cùng + khung soạn tin + panel Catch dạng sheet (cần `viewport-fit=cover` ở `index.html`). Sửa thêm 1 lỗi phát hiện khi kiểm tra bằng ảnh chụp di động: ô soạn tin không tự giãn cao theo nội dung mồi sẵn (câu gợi mở từ "Hôm nay") ở lần render đầu — chỉ giãn khi người dùng tự gõ — khiến câu gợi mở 2 dòng bị cắt mất một phần. |
| 2026-09-17 | Claude (agent) | Đợt 3 cùng ngày — theo phản hồi tiếp tục về mobile: (1) thêm **nút quay lại** (mũi tên `<`, chỉ hiện trên di động) ở góc trên màn Trò chuyện — trước đó không có cách nào rời màn này trên di động ngoài "Kết thúc phiên" hoặc nút back trình duyệt, do thanh tab dưới cùng bị ẩn ở route này để nhường chỗ khung soạn tin; nút mới điều hướng về "Hôm nay" mà **không** kết thúc phiên (phiên vẫn ACTIVE, mở lại được) — xem mục 4.4. (2) Đổi 3 nút thao tác dưới khung soạn tin (Catch/Summarize/End session) sang dạng icon + chữ ngắn, gọn — bỏ chữ "⌘K to catch a line" (nhắc một phím tắt chưa từng được lập trình thật, gây hiểu lầm là nút hỏng) và "End & keep the shift" (người dùng phản ánh khó hiểu) — xem mục 4.4. Không đổi route, model dữ liệu hay endpoint. |
